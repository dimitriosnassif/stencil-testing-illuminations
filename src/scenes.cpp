...
void StencilTestScene::render(glm::mat4& view, glm::mat4& projPersp)
{
    ...
    for (int i = 0; i < N_ALLY_MONKEE; i++)
    {
        ...
        glStencilMask(0x01);
        glUniformMatrix4fv(m_res.mvpLocationModel, 1, GL_FALSE, &allyFinalTransform[i][0][0]);
        m_res.suzanne.draw();
    }
    
    for (int i = 0; i < N_ENEMY_MONKEE; i++)
    {
        glStencilFunc(GL_ALWAYS, 1 << (i + 1), 1 << (i + 1));
        glStencilMask(1 << (i + 1));
        glUniformMatrix4fv(m_res.mvpLocationModel, 1, GL_FALSE, &enemyFinalTransform[i][0][0]);
        m_res.suzanne.draw();
    }
    ...
    glassMat = glm::translate(glassMat, glm::vec3(0.0f, -1.0f, 0.0f));
    ...
    for (int i = 0; i < N_ALLY_MONKEE; i++)
    {
        ...
        glUniform1f(m_res.haloFactorLocationSimple, 0.1f);
        glUniformMatrix4fv(m_res.mvpLocationSimple, 1, GL_FALSE, &allyFinalTransform[i][0][0]);
        m_res.suzanne.draw();
        glDepthFunc(GL_LESS);
